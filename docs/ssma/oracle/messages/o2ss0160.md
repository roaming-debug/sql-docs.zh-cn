...
description: 介绍 Oracle SQL Server 迁移助手 (SSMA) 为何不在具有格式错误的 ORDER BY 子句的两种情况下转换 SELECT 语句。
author: nahk-ivanov
ms.prod: sql
ms.technology: ssma
ms.devlang: sql
...
ms.locfileid: "99187891"
---
# <a name="o2ss0160-select-statement-not-converted-error"></a>O2SS0160： SELECT 语句未转换 (错误) 

本文介绍了对于 Oracle SQL Server 迁移助手 (SSMA) 不会转换包含格式 `SELECT` 错误子句的两种情况下的语句的原因 `ORDER BY` 。
