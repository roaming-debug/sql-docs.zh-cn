...
1. 打开 HTML 文件，并输入以下信息：
    ...
    - **管理员用户名** -服务器管理员用户名。 请确保登录名满足以下要求：
      - 登录名不能包含 SQL 标识符或典型的系统名称 (如 admin、administrator、sa、root、dbmanager、loginmanager 等 ) 或内置数据库用户或角色 (如 dbo、guest、) public 等。
      - 登录名不能包含非字母数字字符 (包括空格、Unicode 字符) 。
      - 登录名不能以数字或符号开头。

    - **管理员密码** -服务器管理员密码。 
      - 密码长度必须至少为8个字符，且长度不能超过128个字符。
...
1. 打开 HTML 文件，并输入以下信息：
    ...
    - **实例管理员用户名** –实例管理员用户名。 请确保登录名满足以下要求：
      - 登录名不能包含 SQL 标识符或典型的系统名称 (如 admin、administrator、sa、root、dbmanager、loginmanager 等 ) 或内置数据库用户或角色 (如 dbo、guest、) public 等。
      - 登录名不能包含非字母数字字符 (包括空格、Unicode 字符) 。
      - 登录名不能以数字或符号开头。

    - **实例管理员密码** -实例管理员密码。 
      - 密码长度必须至少为16个字符，且长度不能超过128个字符。
      - 密码必须含以下字符类别中的三类 – 英文大写字母、英文小写字母、数字(0-9)及非字母数字字符（!、$、#、% 等）。
      - 你的密码不能包含全部或部分登录名。 登录名 (部分定义为三个或更多连续字母数字字符。 ) 
