---
title: SqlPackage 部署报表和偏移报表
description: 了解如何通过 SqlPackage.exe 部署报表和偏移报表自动执行数据库开发任务。 查看示例和可用参数、属性和 SQLCMD 变量。
ms.prod: sql
...
ms.locfileid: "100081498"
---
# <a name="sqlpackage-deploy-report-and-drift-report"></a>SqlPackage 部署报表和偏移报表
SqlPackage.exe [DeployReport](#deployreport-action-parameters) 操作创建将由发布操作完成的更改的 XML 报表。
SqlPackage.exe [DriftReport](#driftreport-action-parameters) 操作创建自注册数据库注册以来已对其做出的更改的 XML 报表。  
...
## <a name="deployreport-action-parameters"></a>DeployReport 操作参数
...
## <a name="deployreport-action-properties"></a>DeployReport 操作属性
...
## <a name="driftreport-action-parameters"></a>DriftReport 操作参数
